/**
 * @file   test_constant_speed_writing_time_ctrl.cc
 * @author Fabian Barras <fabian.barras@epfl.ch>
 * @date   Thu Feb 15 10:23:19 2018
 *
 * @brief  Testing constant speed fracture (time-controlled loading)
 *
 * @section LICENSE
...
int main(int argc, char *argv[]){
  ...
  for (std::vector<bool>::iterator it = write.begin(); it < write.end(); ++it) {
    ...
    if(*it){
      ...
      sim_driver.initConstantSpeed(load, psi, phi, max_s_str, 0.0, _time_control, 0.9, G_length);
    }
    else {
      // Read an existing file to set loading conditions
      sim_driver.initLoadingFromFile(output_folder+load_file, _time_control, load, psi, phi);
    }
